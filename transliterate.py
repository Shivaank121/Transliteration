  ...
  with tf.Session() as sess:
    ...
    while True:
      ...
      for word in words:
        ...
        ans.append("".join([tf.compat.as_str(rev_hn_vocab[output]) for output in outputs]))
        #print (outputs)
        #print("> ", end="")
        #sys.stdout.flush()
        #line = sys.stdin.readline()
...
def evaluate():
  ...
  with tf.Session() as sess:
    ...
    en_eval_path = os.path.join(FLAGS.transliterate_file_dir,'test1.en')
    ...
    f = codecs.open(result_path, encoding='utf-8', mode='ab')
    f.write(fc_str.decode('utf-8'))
    print(fc_str.decode('utf-8'))
